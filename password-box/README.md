...
- **Open source** : les données sensibles sont chiffrées localement.
...
│   ├── index.tsx            # I18nProvider, useI18n() expose { t, tt, lang, setLang, isRTL }
...
**Android natif** (utile pour toucher au splash natif ou aux plugins) :
...
- **`t(clé, {params})`** : clé typée `TranslationKey` (une clé manquante afficherait la clé brute, donc jamais de traduction oubliée silencieusement).
- **`tt(texteOuClé)`** : traduit si c'est une clé connue, sinon renvoie le texte tel quel (par ex. `customType` / `customOS` saisis par l'utilisateur).
...
2. Respectez le style existant et n'ajoutez **aucun commentaire superflu**.
3. Vérifiez : `npx tsc --noEmit` sans erreur, et l'app se recharge sans erreur de bundle.
4. Ouvrez une **pull request** avec une description claire.

Ou plus simple : signalez un bug, proposez une idée, ou aidez à traduire.
